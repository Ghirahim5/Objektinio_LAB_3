...
public:
    ...
    ManoVektorius() : arr(nullptr), capacity(0), current(0) {}
    ...
    ManoVektorius(const ManoVektorius& other) : allocator(other.allocator), arr(nullptr), capacity(0), current(0)
    {
        if (other.current > 0)
        {
    ...
    ManoVektorius& operator=(const ManoVektorius& other)
    {
        if (this != &other)
        {
            ManoVektorius temp(other);
            swap(temp);
        }
        return *this;
    }

    ManoVektorius(ManoVektorius&& other) noexcept : allocator(std::move(other.allocator)), arr(other.arr), capacity(other.capacity), current(other.current)
    {
        other.arr = nullptr;
        other.capacity = 0;
        other.current = 0;
    ...
    reference at(size_type pos)
    {
        if (pos >= current)
        {
            throw std::out_of_range("ManoVektorius::at");
        }
        return arr[pos];
    }
    ...
    void pop_back()
    {
        ...
        {
            throw std::out_of_range("Cannot pop_back from an empty ManoVektorius");
        }
    }
...
bool operator==(const ManoVektorius<T, Allocator>& lhs, const ManoVektorius<T, Allocator>& rhs)
{
...
{
    template <typename T, typename Allocator>
    void swap(ManoVektorius<T, Allocator>& lhs, ManoVektorius<T, Allocator>& rhs) noexcept
    {
        lhs.swap(rhs);
    }
}
...
void erase(ManoVektorius<T, Allocator>& vec, Pred pred)
{
    vec.erase(std::remove_if(vec.begin(), vec.end(), pred), vec.end());
}

template <typename T, typename Allocator, typename Pred>
void erase_if(ManoVektorius<T, Allocator>& vec, Pred pred)
{
    vec.erase(std::remove_if(vec.begin(), vec.end(), pred), vec.end());
}
